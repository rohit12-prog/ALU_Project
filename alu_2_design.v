...
module alu_testbench;
    ...
    aluu #(.N(N),.M(M))
    dut ( .clk(CLK),.rst(RST), .INP_VALID(IN_VAL),.CMD(CMD),.MODE(MODE),.CE(CE),.OPA(OPA),.OPB(OPB),.CIN(CIN), .ERR(ERR_dut),.RES(RES_dut),
.OFLOW(OFLOW_dut), .COUT(COUT_dut),.G(G_dut),.L(L_dut),.E(E_dut));
